...
if modo == "🧠 Recomendador Inteligente":
    st.subheader("🧠 Calculadora Recomendadora")

    dividendo_deseado = st.number_input("¿Cuánto deseas pagar como dividendo mensual? (CLP)", min_value=10000, value=400000, step=10000)
    sueldo = st.number_input("¿Cuál es tu sueldo líquido mensual? (CLP) (opcional)", min_value=0, value=0, step=10000)
    plazo_recom = st.slider("Plazo del crédito (años)", min_value=5, max_value=30, value=20)

    # Asumimos valores razonables
    tasa_anual = 0.04
    seguro_mensual = 10000
    tasa_mensual = (1 + tasa_anual) ** (1/12) - 1
    n_meses = plazo_recom * 12

    dividendo_sin_seguro = max(dividendo_deseado - seguro_mensual, 0)
    
    if tasa_mensual > 0 and dividendo_sin_seguro > 0:
        monto_credito_uf = dividendo_sin_seguro * (1 - (1 + tasa_mensual) ** -n_meses) / tasa_mensual
        pie_recomendado_uf = monto_credito_uf * 0.2
        precio_total_uf = monto_credito_uf + pie_recomendado_uf
    else:
        monto_credito_uf = pie_recomendado_uf = precio_total_uf = 0

    st.markdown(f"🏠 Puedes optar a un crédito de **{monto_credito_uf:,.2f} UF** (~{monto_credito_uf * uf_clp:,.0f} CLP)")
    st.markdown(f"💰 Necesitarás un pie estimado de **{pie_recomendado_uf:,.2f} UF** (~{pie_recomendado_uf * uf_clp:,.0f} CLP)")
    st.markdown(f"💡 El precio máximo de la vivienda sería de **{precio_total_uf:,.2f} UF** (~{precio_total_uf * uf_clp:,.0f} CLP)")

    if sueldo > 0:
        porcentaje = (dividendo_deseado / sueldo) * 100
        if porcentaje <= 25:
            st.success(f"✅ Estás destinando solo el {porcentaje:.1f}% de tu sueldo. ¡Buena proporción!")
        elif porcentaje <= 40:
            st.warning(f"⚠️ Estás destinando el {porcentaje:.1f}% de tu sueldo. Asegúrate de que sea sostenible.")
        else:
            st.error(f"🚨 Estás destinando el {porcentaje:.1f}% de tu sueldo. Es un nivel alto de endeudamiento.")
    ...
    with col1:
        ...
        # Pie mínimo 10% del precio de la vivienda
        pie_uf = st.number_input(
            "Pie inicial (UF) - mínimo 10% del precio de la vivienda",
            value=precio_uf*0.2,
            min_value=precio_uf*0.1,
            max_value=precio_uf
        )
    ...
    if st.button("🔄 Calcular Crédito"):
        ...
        if modo == "Comprador para vivir" and anio_salto:
