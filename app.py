...
# --- Indicadores: UF, TPM, IPC y Dólar ---
try:
...
# Mostrar indicadores en columnas ordenados arriba
c1, c2, c3, c4 = st.columns(4)
c1.metric("UF (CLP)", f"{uf_clp:,.2f}")
c2.metric("TPM (%)", f"{tpm:.2f}" if tpm else "N/A")
c3.metric("IPC (%)", f"{ipc:.2f}" if ipc else "N/A")
c4.metric("Dólar (CLP)", f"{dolar:,.0f}" if dolar else "N/A")
